export default {
    skills: [
      {
        name: "HTML",
        icon: `<i class="fa-brands fa-html5 skill-icons"></i>`,
      },
      {
        name: "CSS",
        icon: `<i class="fa-brands fa-css3 skill-icons"></i>`,
      },
      {
        name: "JavaScript",
        icon: `<i class="fa-brands fa-js skill-icons"></i>`,
      },
      {
        name: "Python",
        icon: `<i class="fa-brands fa-python skill-icons"></i>`,
      },
    ],
    projects: [
      {
        title: "Kingram Lab Alpha",
        liveURL: "https://codepen.io/kobyingram11/full/RwLOxXb",
      },
      {
        title: "Kingram Lab Beta",
        liveURL: "kingram-html-beta-cis-376.zip",
      },
      {
        title: "Kingram Lab Charlie",
        liveURL: "https://kobyingram11.github.io/scaling-palm-tree/",
        srcCodeURL: "https://github.com/kobyingram11/scaling-palm-tree",
      },
      {
        title: "Kingram Lab Delta",
        liveURL: "https://cryptodisco.vercel.app/",
        srcCodeURL: "https://github.com/KhoiUna/cryptodisco-client-vercel",
      },
      {
        title: "Kingram Lab Echo",
        liveURL: "https://cryptodisco.vercel.app/",
        srcCodeURL: "https://github.com/KhoiUna/cryptodisco-client-vercel",
      },
    ],
    contacts: [
      {
        type: "email",
        icon: `<i class="fa-solid fa-envelope"></i>`,
        text: "kingram2@una.edu",
      },
    ],
  
    // Please add .social-icons for the icons
    socials: [
      {
        link: "https://github.com/kobyingram11",
        icon: `<i class="fa-brands fa-github social-icons"></i>`,
      },
      {
        link: "https://www.linkedin.com/in/koby-ingram-273a531b7",
        icon: `<i class="fa-brands fa-linkedin-in social-icons"></i>`,
      },
      {
        link: "https://codepen.io/kobyingram11",
        icon: `<i class="fa-brands fa-codepen social-icons"></i>`,
      },
    ],
  };